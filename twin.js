(function(window){
  ...
  var Phys = S.extend({
  	...
  	draw : function(paperRaph, ref){
  		this.drawing = this.drawRaph(paperRaph, ef)
  	},

  	drawRaph : function(paperRaph, ref){
  		var relativeSpeed = [this.speed[0] - ref.obj.speed[0], this.speed[1] - ref.obj.speed[1]];
  		
  		var size = [lorentz.getContracted(relativeSpeed, ref.limitSpeed, size[0]),lorentz.getContracted(relativeSpeed, ref.limitSpeed, size[1])];

  		var pos = [relativeSpeed[0]*this.ref.time, relativeSpeed[1]*this.ref.time];
